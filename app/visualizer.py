"""Campus card visualization — 4 matplotlib charts, parallel generation."""
import logging
import os
...
def _check_cjk_font():
    """Verify at least one CJK-capable font is available. Logs a warning if not."""
    available = {f.name for f in fm.fontManager.ttflist}
    found = [c for c in _CJK_FONT_CANDIDATES if c in available]
    if found:
        _log.info("检测到可用中文字体: %s", ', '.join(found[:3]))
    else:
        _log.warning("未检测到任何中文字体！图表中文可能显示为方块。"
                     "建议安装 SimHei 或 Noto Sans CJK SC。")
...
def _chart_monthly_trend(expenses_df):
    """Monthly effective daily spending trend line chart."""
    try:
        ...
        _log.exception("生成月度趋势图失败: %s", e)
        print(f"  [FAIL] 月度趋势图: {e}")
...
def _chart_canteen_pie(expenses_df):
    ...
    try:
        ...
        _log.exception("生成食堂占比图失败: %s", e)
        print(f"  [FAIL] 食堂占比图: {e}")
...
def _chart_weekday_comparison(expenses_df):
    ...
    try:
        ...
        _log.exception("生成星期对比图失败: %s", e)
        print(f"  [FAIL] 星期对比图: {e}")


def _chart_hourly_heatmap(expenses_df):
    """Hourly spending heatmap by weekday."""
    try:
        ...
        _log.exception("生成时段热力图失败: %s", e)
        print(f"  [FAIL] 时段热力图: {e}")
...
def run_visualization(file_path):
    """Load data and generate all 4 charts in parallel."""
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(fn, expenses_df): fn.__name__ for fn in _CHART_FUNCTIONS}
        for future in as_completed(futures):
            try:
                future.result()
            except Exception as e:
                _log.exception("图表生成异常 [%s]: %s", futures[future], e)
